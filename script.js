const dropArea=document.getElementById("dropArea"),fileInput=document.getElementById("fileInput"),beforeImg=document.getElementById("before");
...
document.querySelectorAll('#advancedSettings input[type="range"]').forEach(sl=>{
  sl.addEventListener('input',()=>{ if(currentPreset!=="custom"){currentPreset="custom"; document.getElementById("customPreset").classList.add("active");} });
});

// info表示
document.querySelectorAll('.info-icon').forEach(icon=>{
  icon.addEventListener('click',()=>alert(icon.dataset.info));
});
...
document.getElementById("showBefore").addEventListener('click',()=>{
  ...
  document.getElementById("showBefore").classList.add("active");
  document.getElementById("showAfter").classList.remove("active");
});
document.getElementById("showAfter").addEventListener('click',()=>{
  beforeImg.style.display="none"; preview.style.display="block";
  document.getElementById("showAfter").classList.add("active");
  document.getElementById("showBefore").classList.remove("active");
});

// スライダー値表示
document.querySelectorAll('#advancedSettings input[type="range"]').forEach(sl=>{
  const span=document.getElementById(sl.id+"Val");
  if(span){ sl.addEventListener('input',()=>{ span.textContent=sl.value; }); }
});

// SVG最適化
function optimizeSVG(svg){
  return svg.replace(/(\d+\.\d{2})\d+/g,"$1").replace(/\n/g,"").replace(/\s{2,}/g," ").replace(/<path d=""/g,"");
}
...
function convert(){
  ...
  loading?.style.display="block"; preview.innerHTML=""; link.style.display="none";
  ...
  reader.onload=function(e){
    ...
    img.onload=function(){
      ...
      ImageTracer.imageToSVG(img.src,function(svg){
        ...
        link.href=url; link.download="output.svg"; link.style.display="block";
